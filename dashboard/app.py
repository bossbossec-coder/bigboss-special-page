...
col1.metric("本日の売上合計", format_yen(yoy_today["today_total"]))
col2.metric(
    "前年同日比",
...
col4.metric("月末着地予測", format_yen(forecast), help=forecast_note)
...
ranking_display = ranking.rename(
    columns={
        "store": "店舗",
        "mtd_sales": "当月累計売上",
        "last_year_mtd_sales": "前年同期間売上",
        "yoy_pct": "前年比",
    }
)
st.dataframe(
    ranking_display.style.format(
        {"当月累計売上": "¥{:,.0f}", "前年同期間売上": "¥{:,.0f}", "前年比": "{:+.1f}%"}
    ),
